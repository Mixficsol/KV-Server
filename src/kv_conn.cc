...
void Conn::Finterpreter() { // 解析序列化后的数据
  ...
  if (buffer[read_pos] == '*' && instruction_len_ == 0 && flag) {  // 对序列化句子的第一次判断
    order = '*';
    analysis_pos = read_pos = 1;
    is_standard_ = true;
  }
  if (buffer[read_pos] == '$' && order == '$' && flag) {
    read_pos += 1;
    analysis_pos = read_pos;
  }
  if (buffer[read_pos] == '\r' && flag) {  // buffer开头是'\r'则先判断
    Assert();  // 若buffer开头是'\r'则先进行判断
  }
  if (order == '&' && flag) { // 如果上次字符串未读完则继续读
    Add();
  }
  while (is_standard_ && flag) {
    Parsing();
  }
}
...
void Conn::AnalyticData() {
  ...
  n = read(fd_, read_buffer_ + offset_, MAXLINE - offset_);
  Finterpreter(); // 序列化解析函数
  if (n <= 0 && instruction_len_ == 0) {
    is_connect_ = false;
    return;
  }
