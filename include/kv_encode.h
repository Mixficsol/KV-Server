...
class Encode {
 public:
  ...
  static bool Split(const std::string& in, std::vector<std::string>* out, int &cur_pos, int& size); // 使key, value, 指令装到vector里面
static void orderTolower(std::string& order); // 指令小写化
  static void Find$(const std::string& in, int& cur_pos);
  static void Back$(const std::string& in, int& cur_pos);
